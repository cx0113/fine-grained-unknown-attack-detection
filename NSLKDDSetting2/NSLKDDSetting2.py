...
class CVAE1(nn.Module):
    def __init__(self):
        ...
        self.l_z_xy=nn.Sequential(nn.Linear(41+9, 35), nn.Softplus(),nn.Linear(35, 20), nn.Softplus(), nn.Linear(20, 2*4))
        self.l_z_x=nn.Sequential(nn.Linear(41,36),nn.Softplus(), nn.Linear(36,20),nn.Softplus(),nn.Linear(20, 2*4))
        self.l_y_xz=nn.Sequential(nn.Linear(41+4,35),nn.Softplus(), nn.Linear(35,20),nn.Softplus(),nn.Linear(20, 9),nn.Sigmoid())     
...
class CVAE2(nn.Module):
    def __init__(self):
        ...
        self.l_x_yz=nn.Sequential(nn.Linear(4+9,22),nn.Softplus(), nn.Linear(22,32),nn.Softplus(),nn.Linear(32, 41)) 
        self.lb = LabelBinarizer()
...
def test2(test_loader,device,cuda):
    ...
    model = torch.load('minmax_f3_CVAE21_test.pkl')
...
def Stage1Train(train_loader,device,cuda,num_epoch):
  ...
  torch.save(model, 'minmax_f3_CVAE11.pkl')  
  
def Stage2Train(train_loader,device,cuda,num_epoch):
    #Sets the module in training mode.
  model=CVAE2()
  model_dict = model.state_dict()
  pretrained_dict = torch.load('minmax_f3_CVAE11.pkl')
  ...
  #for k,v in model.state_dict().items():
  #   print(k,v)
  torch.save(model, 'minmax_f3_CVAE21_test.pkl') 
...
def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig("confusion_matrix of NSLKDD setting2.jpg")
...
def main(argv):
  ...
  datatrain = pd.read_csv('../DataSample/NSLKDD_train_setting2.csv')
  datatrain = numpy.array(datatrain)
  xtrain = datatrain[:,:41]  
  ytrain = datatrain[:,41]

  testdatatest = pd.read_csv('../DataSample/NSLKDD_test_setting21d.csv')
  testdatatest = np.array(testdatatest)
  xtest = testdatatest[:,:41]
  ytest1 = testdatatest[:,41]
  
  #xtest_t = testdatatest[:,:41]
  #ytest1_t = testdatatest[:,41]
  ...
  #Stage2Train(train_loader,device,cuda_available,num_epoch=400) 
  print("training done")
  ...
  #attack_types=['guess_passwd', 'ipsweep', 'neptune', 'nmap', 'normal', 'portsweep', 'smurf', 'teardrop', 'warezmaster', 'unknown attack']
  #print(confusionmatrix)
  #plot_confusion_matrix(confusionmatrix, classes=attack_types, normalize=True, title='NSL-KDD setting2')
  """
  ...
  #plt.hist(reconstruction_error[ytest1==5][reconstruction_error[ytest1==5]<40], **kwargs)#blue
  ...
  #print(reconstruction_error[ytest1>8][reconstruction_error[ytest1>8]<5].shape)
  #plt.hist(reconstruction_error[(ytest1>8)&(predict==5)][reconstruction_error[(ytest1>8)&(predict==5)]<40], **kwargs)  #red
  ...
  thre=np.array([2.23  ,  1.11  , 0.78 , 0.61  , 0.52])
  ...
  for i in range(5):
      ...
      y_pred1[(predict==2)&(reconstruction_error>4)]=9
      y_pred1[(predict==5)&(reconstruction_error>2)]=9
      y_pred1[(predict==1)&(reconstruction_error>2)]=9
      y_pred1[(predict==3)&(reconstruction_error>2)]=9
      y_pred1[(predict==6)&(reconstruction_error>2)]=9
      y_pred1[(predict==7)&(reconstruction_error>2)]=9
      index=y_true<9
  ...
  np.savetxt("openness_NSLKDDSetting2.txt", openness)  
  
  result=np.zeros((100,8))
  for i in range(100):
    y_pred1=copy.deepcopy(predict)
    y_true1=copy.deepcopy(ytest1)
    y_pred1[(y_pred1==4)&(reconstruction_error>0.0+0.05*i)]=9
    y_pred1[(predict==2)&(reconstruction_error>0.0+0.1*i)]=9
    y_pred1[(predict==5)&(reconstruction_error>0.0+0.1*i)]=9
    y_pred1[(predict==1)&(reconstruction_error>0.0+0.05*i)]=9
    y_pred1[(predict==3)&(reconstruction_error>0.0+0.05*i)]=9
    y_pred1[(predict==6)&(reconstruction_error>0.0+0.1*i)]=9
    y_pred1[(predict==7)&(reconstruction_error>0.0+0.05*i)]=9
    known=y_pred1[y_true1<9]
    unknown=y_pred1[y_true1>8]
    y_true1[y_true1>8]=9
  ...
  np.savetxt("report_NSLKDDSetting2.txt", result) 
